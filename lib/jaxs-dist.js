...
const shouldCompileChildren = (baseInstructions)=>{
    ...
    return instruction.type !== ChangeInstructions.removeNode && instruction.type !== ChangeInstructions.replaceNode;
};
const compileForDom = (source, target)=>{
    if (source.nodeType !== target.nodeType) {
        ...
        if (shouldCompileChildren(baseInstructions)) {
            childrenInstructions = compileForCollection(sourceElement.childNodes, targetElement.childNodes, sourceElement);
        }
