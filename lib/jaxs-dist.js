...
const recursiveRender = (children, renderKit, rendered = []) =>
  children.reduce(renderReducer(renderKit), rendered).flat();
...
(function (ChangeInstructions) {
  ChangeInstructions[ChangeInstructions['removeNode'] = 0] = 'removeNode';
  ChangeInstructions[ChangeInstructions['insertNode'] = 1] = 'insertNode';
  ChangeInstructions[ChangeInstructions['replaceNode'] = 2] = 'replaceNode';
  ChangeInstructions[ChangeInstructions['removeAttribute'] = 3] =
    'removeAttribute';
  ChangeInstructions[ChangeInstructions['addAttribute'] = 4] = 'addAttribute';
  ChangeInstructions[ChangeInstructions['updateAttribute'] = 5] =
    'updateAttribute';
  ChangeInstructions[ChangeInstructions['removeEvent'] = 6] = 'removeEvent';
  ChangeInstructions[ChangeInstructions['addEvent'] = 7] = 'addEvent';
  ChangeInstructions[ChangeInstructions['updateEvent'] = 8] = 'updateEvent';
  ChangeInstructions[ChangeInstructions['changeValue'] = 9] = 'changeValue';
  ChangeInstructions[ChangeInstructions['changeText'] = 10] = 'changeText';
...
const compileForElement = (source, target) => {
  ...
  return attributeInstructions.concat(eventInstructions).concat(
    valueInstructions,
  );
};
...
const compileForNodeGenerator = (compileForCollection) => (source, target) => {
  let instructions = [];
...
const compileChildren = (sourceList, targetList, parent) => {
...
const instructionsSorter = (left, right) => {
  if (left.type > right.type) return 1;
  if (left.type < right.type) return -1;
  return 0;
};
const largerLength = (sourceList, targetList) => {
...
const performers = {
  ...
  [ChangeInstructions.insertNode]: insertNode1,
...
function isDraftable(value) {
  ...
  return isPlainObject(value) || Array.isArray(value) || !!value[DRAFTABLE] ||
    ...
    this.produce = (base, recipe, patchListener) => {
      if (typeof base === 'function' && typeof recipe !== 'function') {
        ...
        return function curriedProduce(base2 = defaultBase, ...args) {
          return self.produce(
            base2,
            (draft) => recipe.call(this, draft, ...args),
          );
        };
      }
...
const routeChangeEvent = 'routeChange';
...
const onLocationChange = (_payload, { publish, setState }) => {
...
const linkNavigationEvent = 'goToHref';
const programmaticNavigationEvent = 'navigate';
const navigate = (path, { publish }) => {
  history.pushState(null, '', path);
  publish(locationChangeEvent);
};
const onLinkClick = (domEvent, { publish }) => {
...
const setupRenderKit = (app, document) => {
  app.renderKit = {
    publish: app.publish,
...
const createApp = (document = window.document) => {
  const app = {};
  setupBus(app);
  setupState(app);
...
const If = ({ condition, children }) => {
  if (!condition) return;
  return jsx(jsx.fragment, null, children);
};
...
const IfElse = ({ condition, children }) => {
  ...
  if (condition) return jsx(jsx.fragment, null, first);
  return jsx(jsx.fragment, null, rest);
};
const mod1 = {
  If: If,
  Unless: Unless,
